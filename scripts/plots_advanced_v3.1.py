...
# -*- coding: utf-8 -*-
"""
plots_advanced_v3_1.py
Improved final version — fixes scatter crash, larger cleaner plots, skips empty hues, removes warnings.
"""
...
OUTDIR = "analysis/plots_v3_1"
os.makedirs(OUTDIR, exist_ok=True)
sns.set_theme(style="whitegrid", font_scale=1.45)
plt.rcParams.update({
    "figure.figsize": (14, 9),
    "savefig.dpi": 300,
    "axes.titlesize": 20,
...
def rob_to_size(series):
    """Map ROB size numeric column to consistent bubble sizes."""
    if series.isna().all(): return np.repeat(100, len(series))
    try:
        s = pd.to_numeric(series, errors="coerce")
        rmin, rmax = np.nanmin(s), np.nanmax(s)
        if rmax == rmin:
            return np.repeat(120, len(s))
        norm = (s - rmin) / (rmax - rmin)
        return 80 + norm * 200
    except Exception:
        return np.repeat(100, len(series))

def add_rob_legend(ax, rob_vals):
    """Add custom legend for representative ROB sizes."""
    from matplotlib.lines import Line2D
    handles, labels = [], []
    for r in sorted(set(rob_vals))[:3]:
...
def plot_scatter_ipc_vs_l1d():
    """Robust scatter: IPC vs L1D miss rate."""
    if not {"IPC","L1D_miss_rate"}.issubset(df.columns): return
    sizes = rob_to_size(df.get("rob", pd.Series([np.nan]*len(df))))
    fig, ax = plt.subplots(figsize=(14,9))
    sns.scatterplot(data=df, x="L1D_miss_rate", y="IPC",
                    hue="benchmark" if "benchmark" in df else None,
                    style="benchmark" if "benchmark" in df else None,
                    s=sizes.astype(float),
                    alpha=0.85, palette=PALETTE, ax=ax, legend="brief")
    ax.set_title("IPC vs L1D Miss Rate — Memory Locality Impact (size ≈ ROB)")
    ax.set_xlabel("L1D Miss Rate (%)"); ax.set_ylabel("IPC")
    if "rob" in df and df["rob"].notna().any():
        add_rob_legend(ax, df["rob"].dropna().unique())
...
def plot_issuewidth_ipc():
    ...
    sns.lineplot(data=df, x="issue_width", y="IPC",
...
def plot_memdep_ipc():
    ...
    sns.barplot(data=df, x="memdep", y="IPC",
                hue="benchmark" if "benchmark" in df else None,
...
def plot_assoc_vs_hit():
    ...
    fig, ax = plt.subplots(figsize=(13,8))
    sns.lineplot(data=df, x="assoc", y="HitRate_L1D",
                 hue="benchmark" if "benchmark" in df else None,
                 marker="o", palette=PALETTE, ax=ax, errorbar=None)
...
# --- EXECUTE ---
plot_scatter_ipc_vs_l1d()
plot_issuewidth_ipc()
plot_memdep_ipc()
plot_assoc_vs_hit()

log("✅ v3.1 plots generated successfully.")
